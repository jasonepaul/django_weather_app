...
def get_dataset(source_df, name, distribution):
    df = source_df[source_df.airport == name].copy()
    del df['airport']
...
def update_plot():
    ...
    src = get_dataset(weather_df, cities[city]['airport'], distribution_select.value)
...
weather_df = pd.read_csv(join(dirname(__file__), 'data/2015_weather.csv'))
source = get_dataset(weather_df, cities[city]['airport'], distribution)
